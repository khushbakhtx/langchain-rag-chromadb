...
try:
    ...
    pass  # Fallback to default sqlite3 if pysqlite3-binary is not installed
    ...
    for csv_path in csv_paths:
        try:
            ...
            if df.empty or df.columns.empty:
                st.warning(f"Empty or invalid CSV: {csv_path}")
                continue
            for idx, row in df.iterrows():
                row_content = ", ".join([f"{col}: {val}" for col, val in row.items() if pd.notnull(val)])
                if not row_content:
                    continue
                doc = Document(
...
def create_vector_store(documents: List[Document]) -> Chroma:
    try:
        ...
        # Use /tmp/chroma_db for Streamlit Cloud compatibility
...
def create_table_query_tool() -> Tool:
    return Tool(
        name="table_query",
        func=table_query_tool,
        description="Analyze tabular CSV data for queries, including ARPU forecasts."
    )
...
def create_retriever_tool_instance(vector_store: Chroma) -> Tool:
    try:
        ...
        return create_retriever_tool(
            retriever,
            "retrieve_documents",
            "Search tabular CSV data for relevant rows, especially revenue and user data."
        )
    except Exception as e:
...
def create_agent(vector_store: Chroma) -> AgentExecutor:
    try:
        llm = ChatOpenAI(model_name="gpt-4o-mini", api_key=openai_api_key, temperature=0)
        tools = [tool for tool in [create_retriever_tool_instance(vector_store), create_table_query_tool()] if tool]
        
        prompt = PromptTemplate(
            template="You are an assistant for querying tabular CSV data. Use 'retrieve_documents' to fetch relevant rows, then 'table_query' to analyze them. For ARPU forecasts, retrieve revenue and user data and pass it to 'table_query'. Provide a single, concise answer. If data is insufficient, state so and suggest what's needed. Avoid excessive tool calls.\n\nChat History: {chat_history}\n\nQuestion: {question}\n\n{agent_scratchpad}",
...
def main():
    st.title("CSV Data Query App")
    st.write("Chat with the assistant to query tabular data from four predefined CSV files.")

    # Hardcoded CSV file paths (replace with your actual file paths)
    csv_paths = [
        "csv_data/file1.csv",
        "csv_data/file2.csv",
        "csv_data/file3.csv",
    ...
    # Process CSV files with loading indicator
    if not st.session_state.is_initialized:
        with st.spinner("Processing CSV files and creating vector store..."):
